...
OTEL_SERVICE_NAME="hadoop-insertindex-job-client"
...
if [ ! -f "trace-insertindex-job.jar" ]; then
  echo "Error: trace-insertindex-job.jar not found. Run build.sh first."
  exit 1
fi
...
OUTPUT_DIR="hdfs:///user/$(whoami)/insertindex-output-$(date +%s)"

echo "--- Job Configuration ---"
echo "Running InsertIndex Job:"
...
export HADOOP_CLIENT_OPTS="$HADOOP_CLIENT_OPTS \
  ...
  -Dotel.baggage.trace.job.id=STATIC_INSERTINDEX_JOB_ID \
...
#    -D insertindex.slow.map.eachN=20 \

#    变慢的reduce
#    -D insertindex.slow.reduce.partitions=0,1,2,3,4 \
#    -D insertindex.slow.reduce.sleep.ms=3000 \
#    -D insertindex.slow.reduce.eachN=1 \

$HADOOP_HOME/bin/hadoop jar trace-insertindex-job.jar \
    org.example.TraceInsertIndexJob \
    -libjars "$OTEL_LIBJARS" \
    -D insertindex.slow.map.partitions=0,1 \    # 插入处
    -D insertindex.slow.map.sleep.ms=2000 \     # 插入处
    -D insertindex.slow.map.eachN=1 \           # 插入处
    -D mapreduce.map.java.opts="-javaagent:${OTEL_AGENT_PATH} -Dotel.service.name=hadoop-insertindex-map -Dotel.traces.exporter=zipkin -Dotel.exporter.zipkin.endpoint=${OTEL_ZIPKIN_ENDPOINT} -Dotel.metrics.exporter=none -Dotel.logs.exporter=none -Dotel.propagators=tracecontext,baggage" \
    -D mapreduce.reduce.java.opts="-javaagent:${OTEL_AGENT_PATH} -Dotel.service.name=hadoop-insertindex-reduce -Dotel.traces.exporter=zipkin -Dotel.exporter.zipkin.endpoint=${OTEL_ZIPKIN_ENDPOINT} -Dotel.metrics.exporter=none -Dotel.logs.exporter=none -Dotel.propagators=tracecontext,baggage" \
    /user/$(whoami)/insertindex-input \
    ${OUTPUT_DIR}
